...
export class TransformerAPI {
  ...
  public async run(): Promise<void> {
    ...
    try {
      const result = await wasm.WebAssembly.instantiate(
        wasmCode,
      ...
      this.ImageTransformHost = result.instance.exports.ImageTransformHost as (
        targetHost: string,
        srcReference: string
      ) => string;
    } catch (err) {
  ...
  transformAllContainers(pod: PeprRequest<a.Pod>, address: string) {
    if (pod.Raw?.spec?.initContainers !== undefined) {
      pod.Raw.spec.initContainers.map(container => {
        try {
          container.image = this.imageTransformHost(address, container.image);
        } catch (err) {
  ...
  imageTransformHost(targetHost: string, srcReference: string): string {
